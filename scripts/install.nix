    ...
    }:
    {
      packages.install = pkgs.writeShellApplication {
        ...
        text = ''
          ...
          # Config constants
          readonly CONFIG_REPO="${config.flake.meta.gitHubUsername}/nixosdots" # Dotfile config repo name
          ...
          readonly CONFIG_DIR="$MOUNT_DIR$PERSIST_DIR/etc/nixos"               # Config location in persistent partition
